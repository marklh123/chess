...
from Chess_GUI_backend import white, black, letter_index, index_letter, log_move, Castling, \
    is_opponent_king_in_check, is_checkmate_opponent, is_your_king_in_check, PawnPromotion
import MyConstants
from MyConstants import pos_to_pixel


def get_pixels(pos):

    row_pixel = MyConstants.pos_to_pixel[pos[0]]
    col_pixel = MyConstants.pos_to_pixel[pos[1]]
...
def pixel_to_board(x_pixel, y_pixel):
    ...
    col = x_pixel // MyConstants.square_size  # 0–7 (pixel # / 100) rounded down = horizontal pos of square
    row = y_pixel // MyConstants.square_size  # 0–7 (pixel # / 100) rounded down = vertical pos of square

    col_letter = index_letter.get(col)
    return row, col_letter

pygame.init()
screen = MyConstants.window_size
...
while MyConstants.is_running:
    player = players[active_player_index]

    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            print(f"Saving {len(log_records)} records")  # add this
            ...
            if selected_piece is None:
                ...
                if target in available_moves:
                    ...
                    # check for castling
                        ...
                        result = Castling(
                            black, white, selected_piece,
                            letter_index, index_letter,
                    ...
                    for op in opp:
                        if op["pos"] == [clicked_row, clicked_col]:
                            opp.remove(op)
                            break
                    ...
                    if check_:
                        ...
                        if checkmate:
                            print("Yikes mate its checkmate")
                    else:
    ...
    screen.blit(MyConstants.board_surface, (0, 0))
    screen.blit(MyConstants.surface_turn_background, (0, 800))

    for move in available_moves:
        # highlight available squares in yellow
        highlight = pygame.Surface((MyConstants.square_size, MyConstants.square_size),
                                   pygame.SRCALPHA)  # SRCALPHA makes surface transparent
        ...
        screen.blit(highlight, (px - 25, py - 25))  # highlighted path is actually now centered

        # highlight under selected piece in red
        square_row = MyConstants.pos_to_pixel[clicked_row]
        square_col = MyConstants.pos_to_pixel[letter_index[clicked_col]]

        cancel_move = pygame.Surface((MyConstants.square_size, MyConstants.square_size), pygame.SRCALPHA)
    ...
    for piece in black + white:
        ...
        if piece["color"] == "black" and piece["type"] == "king":
            if black_in_check:
                ...
                check_king_surface = pygame.Surface((MyConstants.square_size, MyConstants.square_size),
                                                    pygame.SRCALPHA)
    ...
    #check text
    if checkmate:
        turn_text = MyConstants.font_turn.render(f"Game over! {you_name} wins by checkmate!", True, "Black")
    elif black_in_check or white_in_check:
        turn_text = MyConstants.font_turn.render(f"{player}'s turn, you are in check!", True, "Black")
    else:
        turn_text = MyConstants.font_turn.render(f"It's {player}'s turn.", True, "Black")
